...
import { cyberSuccessStories } from "@/data/capabilities/cybersecurity";
...
export default function CyberSuccessStoriesSection() {
  ...
  const story = cyberSuccessStories[activeIndex];

  const goPrev = () =>
    setActiveIndex(
      (current) =>
        (current - 1 + cyberSuccessStories.length) % cyberSuccessStories.length
    );
  const goNext = () =>
    setActiveIndex((current) => (current + 1) % cyberSuccessStories.length);
