import {
  getAppSolutionHeroImage,
  getAppSolutionImages,
} from "./appSolutionImages";
import { getStudioTrustedBrands } from "./studioBrands";
import type { StudioPageContent } from "./types";

/** Fallback only when a numbered file is not yet in the folder. */
...
function cardImage(
  ...
  return images[index] ?? FALLBACK[index] ?? "";
}

/** Rebuilds from disk each call so new/replaced folder assets apply immediately. */
export function getAppSolutionsContent(): StudioPageContent {
  const appImages = getAppSolutionImages();
  const heroFromFolder = getAppSolutionHeroImage();
  ...
  return {
    ...
    faqs: [
      {
        question:
          "Can you integrate with our legacy systems without replacing them?",
        answer:
...
/** @deprecated Prefer getAppSolutionsContent() so folder assets stay fresh. */
export const appSolutionsContent = getAppSolutionsContent();
